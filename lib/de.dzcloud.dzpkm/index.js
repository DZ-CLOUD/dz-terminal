export function processCommand(command) {
    ...
    let ourPackage = libs.find(a => a.command === args[0]);
    let searchArg = args[1];
    if (args[0] === "sudo") {
        ourPackage = libs.find(a => a.command === args[1]);
        searchArg = args[2];
    }
    ...
    switch (searchArg) {
        ...
        case "help":
            ...
            break;
        case "-v":
            printToTerminal(`DZ CLOUD Package Menager is version: ${ourPackage["package.version"]} in branch ${ourPackage["package.branch"]}`);
            break;
        default:
