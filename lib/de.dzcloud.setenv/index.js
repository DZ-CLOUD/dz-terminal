export function processCommand(command) {
    ...
    if (args[3].includes("sudo") && isSudo === "false" && isSudo !== "true") {
        printToTerminal(`Can't change sudo variables. Access denied!`);
        return;
    }
    if (args[3].includes("passowrd") && isSudo === "false" && isSudo !== "true") {
...
function setVariable(command, lib, key, value) {
    ...
    if (key.includes("sudo") && isSudo === "false" && isSudo !== "true") {
        printToTerminal(`Can't change sudo variables. Access denied!`);
        return;
    }
    if (key.includes("password") && isSudo === "false" && isSudo !== "true") {
        printToTerminal(`Can't change sudo variables. Access denied!`);
        return;
    }
